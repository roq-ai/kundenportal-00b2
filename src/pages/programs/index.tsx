...
import { getPrograms, deleteProgramById } from 'apiSdk/programs';
import { ProgramInterface } from 'interfaces/program';

type ColumnType = ColumnDef<ProgramInterface, unknown>;
...
export function ProgramListPage(props: ProgramListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ProgramInterface>>(
    () => `/programs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ProgramInterface) => {
    if (hasAccess('program', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/programs/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/programs/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('program', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/programs/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('program', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="program" />
        </Flex>

        {hasAccess('program', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/programs/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
