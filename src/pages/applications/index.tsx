...
export function ApplicationListPage(props: ApplicationListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getApplications({
        relations: ['program', 'user', 'funding.count', 'review.count'],
  ...
  const columns: ColumnType[] = [
    ...
    },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.approval_date ? format(parseISO(record?.approval_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    {
      id: 'rejection_date',
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.rejection_date ? format(parseISO(record?.rejection_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    hasAccess('program', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'program',
          header: 'Program',
          accessorKey: 'program',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/programs/view/${record.program?.id}`}>
              {record.program?.name}
            </Link>
          ),
        }
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('funding', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'funding',
          header: 'Funding',
          accessorKey: 'funding',
          cell: ({ row: { original: record } }: any) => record?._count?.funding,
        }
      : null,
