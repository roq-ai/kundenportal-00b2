...
function DashboardPage() {
  ...
  const programStat = useSWR<PaginatedInterface<ProgramInterface>>(
    () => `/programs`,
    () => getPrograms({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Programs',
      result: programStat?.data?.totalCount,
      isLoading: programStat.isLoading,
